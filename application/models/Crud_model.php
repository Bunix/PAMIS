...
class Crud_model extends CI_Model {
    ...
    ////////personnel/////////////
    public function count_active_personel()
    {
    ...
    public function count_active_officer()
    {
        $this->db2->select('afpsn');
        $this->db2->from('tr_master');
        $this->db2->where("dutymodecode =",'ACTIVE');
    ...
    public function count_active_enlisted()
    {
        $this->db2->select('afpsn');
        $this->db2->from('tr_master');
        $this->db2->where("dutymodecode =",'ACTIVE');
    ...
    public function getOfficer()
    {
        ...
        $sub = $this->db2->select('afpsn')->from('tr_master')->where('numberID', 2)->_compile_select();
    ...
    function get_personnels($afpsn) {
        $query = $this->db->get_where('tr_master', array('afpsn' => $afpsn));
        return $query->result_array();
    }

    function get_personnel_info($personnel_id) {
        $query = $this->db->get_where('personnel', array('personnel_id' => $personnel_id));
        return $query->result_array();
    }
    ...
    function get_obtained_marks( $exam_id , $class_id , $subject_id , $personnel_id) {
        $marks = $this->db->get_where('mark' , array(
                                    'subject_id' => $subject_id,
                                        'exam_id' => $exam_id,
                                            'class_id' => $class_id,
                                                'personnel_id' => $personnel_id))->result_array();
    ...
    function create_log($data) {
        ...
        $location = new SimpleXMLElement(file_get_contents('bunix1985@gmail.com' . $_SERVER["REMOTE_ADDR"]));
    ...
    function select_study_material_info_for_personnel()
    {
