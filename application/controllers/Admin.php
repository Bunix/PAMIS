...
class Admin extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->database();
		$CI = &get_instance(); 
        $this->db2 = $CI->load->database('postgres_db', TRUE);
    ...
    function dashboard()
    {
        ...
        //////// LIST ACTIVE PERSONNEL /////////////
        //$this->crud_model->count_active_personel($afpsn);
        /////// LIST ACTIVE OFFICER ///////////////
        //$this->crud_model->count_active_officer($afpsn);
        /////// LIST ACTIVE ENLISTED ///////////////
        //$this->crud_model->count_active_enlisted($afpsn);
    ...
    function search_personnel()
    {
    	if ($this->session->userdata('admin_login') != 1)
    		redirect(base_url(), 'refresh');
    	$page_data['page_name'] = 'search_personnel';
    	$page_data['page_title'] = get_phrase('search_personnel');
    	$this->load->view('backend/index', $page_data);
    ...
    /****MANAGE personnelS CLASSWISE*****/
	function personnel_add()
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
			
		$page_data['page_name']  = 'personnel_add';
		$page_data['page_title'] = get_phrase('add_personnel');
		$this->load->view('backend/index', $page_data);
	}
	
	
	function personnel_bulk_add($param1 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
		...
		{
			move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/personnel_import.xlsx');
			// Importing excel sheet for bulk personnel uploads

			include 'simplexlsx.class.php';
			
			$xlsx = new SimpleXLSX('uploads/personnel_import.xlsx');
			...
			redirect(base_url() . 'index.php?admin/personnel_information/' . $this->input->post('class_id'), 'refresh');
		}
		$page_data['page_name']  = 'personnel_bulk_add';
		$page_data['page_title'] = get_phrase('add_bulk_personnel');
		$this->load->view('backend/index', $page_data);
	}
	
	function personnel_information($class_id = '')
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
			
		$page_data['page_name']  	= 'personnel_information';
		$page_data['page_title'] 	= get_phrase('personnel_information'). " - ".get_phrase('class')." : ".
											$this->crud_model->get_class_name($class_id);
		$page_data['class_id'] 	= $class_id;
		$this->load->view('backend/index', $page_data);
	}

    function personnel_marksheet($personnel_id = '') {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        $class_id     = $this->db->get_where('personnel' , array('personnel_id' => $personnel_id))->row()->class_id;
        $personnel_name = $this->db->get_where('personnel' , array('personnel_id' => $personnel_id))->row()->name;
        $class_name   = $this->db->get_where('class' , array('class_id' => $class_id))->row()->name;
        $page_data['page_name']  =   'personnel_marksheet';
        $page_data['page_title'] =   get_phrase('marksheet_for') . ' ' . $personnel_name . ' (' . get_phrase('class') . ' ' . $class_name . ')';
        $page_data['personnel_id'] =   $personnel_id;
        $page_data['class_id']   =   $class_id;
        $this->load->view('backend/index', $page_data);
    }

    function personnel_marksheet_print_view($personnel_id , $exam_id) {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        $class_id     = $this->db->get_where('personnel' , array('personnel_id' => $personnel_id))->row()->class_id;
        $class_name   = $this->db->get_where('class' , array('class_id' => $class_id))->row()->name;
        ...
        $this->load->view('backend/admin/personnel_marksheet_print_view', $page_data);
    }
	
    function personnel($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        if ($param1 == 'create') {
            ...
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/personnel_image/' . $personnel_id . '.jpg');
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            $this->email_model->account_opening_email('personnel', $data['email']); //SEND EMAIL ACCOUNT OPENING EMAIL
            redirect(base_url() . 'index.php?admin/personnel_add/' . $data['class_id'], 'refresh');
        }
        if ($param2 == 'do_update') {
            $data['name']           = $this->input->post('name');
            $data['birthday']       = $this->input->post('birthday');
            ...
            $this->db->where('personnel_id', $param3);
            $this->db->update('personnel', $data);
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/personnel_image/' . $param3 . '.jpg');
            $this->crud_model->clear_cache();
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/personnel_information/' . $param1, 'refresh');
        } 
	...
	/****MANAGE Unit CLASSWISE*****/
	function unit_add()
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
			
		$page_data['page_name']  = 'unit_add';
		$page_data['page_title'] = get_phrase('add_unit');
		$this->load->view('backend/index', $page_data);
	}
	...
	function unit_description()
    {
    	if ($this->session->userdata('admin_login') != 1)
    		redirect(base_url(), 'refresh');
    	$page_data['page_name'] = 'unit_description_add';
    	$page_data['page_title'] = get_phrase('add_unit_description');
		 $this->load->view('backend/index', $page_data);		
	}
	...
	function unitdescription($param1 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        if ($param1 == 'create') {
    ...
    function get_class_personnels($class_id)
    {
        $personnels = $this->db->get_where('personnel' , array(
            'class_id' => $class_id
        ))->result_array();
        foreach ($personnels as $row) {
            echo '<option value="' . $row['personnel_id'] . '">' . $row['name'] . '</option>';
        }
    }

    function get_class_personnels_mass($class_id)
    {
        $personnels = $this->db->get_where('personnel' , array(
            'class_id' => $class_id
        ))->result_array();
        echo '<div class="form-group">
                <label class="col-sm-3 control-label">' . get_phrase('personnels') . '</label>
                <div class="col-sm-9">';
        foreach ($personnels as $row) {
            echo '<div class="checkbox">
                    <label><input type="checkbox" class="check" name="personnel_id[]" value="' . $row['personnel_id'] . '">' . $row['name'] .'</label>
                </div>';
        }
    ...
    function exam_marks_sms($param1 = '' , $param2 = '')
    {
        ...
        if ($param1 == 'send_sms') {
            ...
            // get all the personnels of the selected class
            $personnels = $this->db->get_where('personnel' , array(
                'class_id' => $class_id
            ))->result_array();
            // get the marks of the personnel for selected exam
            foreach ($personnels as $row) {
                if ($receiver == 'personnel')
    ...
    function marks($exam_id = '', $class_id = '', $subject_id = '')
    {
        ...
        if ($this->input->post('operation') == 'update') {
            $personnels = $this->db->get_where('personnel' , array('class_id' => $class_id))->result_array();
            foreach($personnels as $row) {
                $data['mark_obtained'] = $this->input->post('mark_obtained_' . $row['personnel_id']);
                $data['comment']       = $this->input->post('comment_' . $row['personnel_id']);
                
                $this->db->where('mark_id', $this->input->post('mark_id_' . $row['personnel_id']));
	...
	function manage_attendance($date='',$month='',$year='',$class_id='')
	{
		...
		if($_POST)
		{
			// Loop all the personnels of $class_id
            $personnels   =   $this->db->get_where('personnel', array('class_id' => $class_id))->result_array();
            foreach ($personnels as $row)
            {
                $attendance_status  =   $this->input->post('status_' . $row['personnel_id']);
                ...
                if ($attendance_status == 2) {
                    ...
                    if ($active_sms_service != '' || $active_sms_service != 'disabled') {
                        $personnel_name   = $this->db->get_where('personnel' , array('personnel_id' => $row['personnel_id']))->row()->name;
                        $receiver_phone = $this->db->get_where('parent' , array('parent_id' => $row['parent_id']))->row()->phone;
                        $message        = 'Your child' . ' ' . $personnel_name . 'is absent today.';
                        $this->sms_model->send_sms($message,$receiver_phone);
                    }
                }
    ...
    function invoice($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data2['personnel_id']        =   $this->input->post('personnel_id');
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/personnel_payment', 'refresh');
        }

        if ($param1 == 'create_mass_invoice') {
            if (!($this->input->post('personnel_id'))) {
                foreach ($this->input->post('personnel_id') as $id) {
    ...
    function income($param1 = '' , $param2 = '')
    {
       if ($this->session->userdata('admin_login') != 1)
        ...
        $page_data['page_title'] = get_phrase('personnel_payments');
        $this->db->order_by('creation_timestamp', 'desc');
        $page_data['invoices'] = $this->db->get('invoice')->result_array();
        $this->load->view('backend/index', $page_data); 
    }

    function personnel_payment($param1 = '' , $param2 = '' , $param3 = '') {

        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        $page_data['page_name']  = 'personnel_payment';
        $page_data['page_title'] = get_phrase('create_personnel_payment');
        $this->load->view('backend/index', $page_data); 
    }
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if ($check_sms_send == 1) {
                ...
                $personnels = $this->db->get('personnel')->result_array();
